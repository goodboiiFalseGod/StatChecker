    ...
    public class Memory
    {
        ...
        public class ExecuteBufferFunctionStatic
        {
            ...
            public void WriteFunction(byte[] array, byte[] argument, int Size1, int Size2)
            {
                ...
                if (address != IntPtr.Zero)
                {
                    WriteBytes(address, array);
                }
            }

            public void StartThread()
            {
                var threadHandle = Kernel32.CreateRemoteThread(ProcessHandle, IntPtr.Zero, 0, this.Address, IntPtr.Zero, 0, out var threadId);
            ...
            public void FreeMemory()
            {
                if (ThreadHandle != IntPtr.Zero)
                {
                    Kernel32.WaitForSingleObject(ThreadHandle, 100);
                }
                Kernel32.VirtualFreeEx(ProcessHandle, this.Address, this.Size1, 2);
                Kernel32.VirtualFreeEx(ProcessHandle, this.BufferAddress, this.Size2, 2);
            }
